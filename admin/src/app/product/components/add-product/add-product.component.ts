...
export class AddProductComponent implements OnInit {
    ...
    onCategoryChange(id:number) {
      if(this.categories) {
       ...
       this.itemDetailsType[0] = []
       this.subCategoriesList = [...data[0].subCategories.map((c:any) => {
    ...
    get qwt() { return this.addProductForm.get('qwt'); }
    get price() { return this.addProductForm.get('price'); }
    get subCategory() { return this.addProductForm.get('subCategory'); }
    get category() { return this.addProductForm.get('category'); }
    get available() { return this.addProductForm.get('available'); }

    get f() { return this.addProductForm.controls; }
    get t() { return this.f.details as FormArray; }

    onProductAdd(values:ProductModel) {
        this.isLoading = true;
        console.log(values);
        this._service.addProduct(values)
        .subscribe(res => {
